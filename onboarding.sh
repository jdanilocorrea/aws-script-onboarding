...
function get-credentials {
  ...
  if [[ "$CONSOLE_URL" == "pnb-hml" ]]; then
      ...
      display_message "Gerar informações arquivo 'credentials' usuário $1 - None."  
      {
      echo "[none]" 
      echo "aws_access_key_id = $1" 
      echo "aws_secret_access_key = $2" 
...
#------------------------------------------------------------------------------
  ...
  read -r -a groups <<<"($(aws iam list-groups --query 'Groups[*].GroupName' --output text))"
  #mapfile -t groups < <"($(aws iam list-groups --query 'Groups[*].GroupName' --output text))"
